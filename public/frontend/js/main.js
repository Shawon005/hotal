...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        $("#booking-form, #horizontal_booking_form, #booking_form_advanced").on('submit', function (e) {
            ...
            var book_error ='Some Thing Is Worng';

            let checkin=new Date(booking_checkin);
            
            let checkin1=moment(checkin).format('YYYYMMDD');
            booking_checkin=moment(checkin).format('DD/MM/YYYY');

            let checkout=new Date(booking_checkout);

            let checkout1=moment(checkout).format('YYYYMMDD');
            booking_checkout=moment(checkout).format('DD/MM/YYYY');

            checkin1=parseInt(checkin1);
            checkout1=parseInt(checkout1);
            ...
            {
                output = '<div class="notification error"><span class="notification-icon"><i class="fa fa-exclamation" aria-hidden="true"></i></span><span class="notification-text"> Departure Date Should be Greater than Arrival Date</span></div>';
            ...
            // if(booking_email==null || booking_roomtype==null || booking_children==null || booking_checkout==null){
            //     output = '<div class="notification success"><span class="notification-icon"><i class="fa fa-check" aria-hidden="true"></i></span><span class="notification-text">' + book_error + '</span></div>';
            //     $("#notification").html(output); 
            ...
            $.post('/bookings/store', post_data, function (response) {
                ...
                {
                    output = '<div class="notification error"><span class="notification-icon"><i class="fa fa-exclamation" aria-hidden="true"></i></span><span class="notification-text">' + response.message + '</span></div>';
                }
                else {
        ...
        $("#contact-form, #contact-form-page").on('submit', function (e) {
            ...
            //Ajax post data to server
            $.post('reviews/store', post_data, function (response) {
       ...
       $("#subscribe-form").on('submit', function (e) {
        ...
        function initialize() {
            ...
            var hotel_address = "Johan Skyttes Väg 190 125 34 Älvsjö";
